...
document.addEventListener('DOMContentLoaded', () => {
    loadDatabase('quiz_v42.db');  // You can specify 'quiz-v1.db' or any version
});
...
function createCourseSelection(courses) {
    ...
    .then(data => {
        console.log("Scores:", data.scores); // Handle the received data
        // You can now display the scores on your static app
    })
    .catch(error => {
...
function startQuiz(dataSource, isView = false) {
    ...
    if (results.length > 0) {
        const questions = results[0].values.map(row => ({
            ...
            importance: row[4],  // importance from the view or query
