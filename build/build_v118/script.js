...
document.addEventListener('DOMContentLoaded', () => {
    // Extract version from dbBaseUrl if possible, else fallback to quiz_v118.db
    let dbName = 'quiz_v118.db';
    const match = dbBaseUrl.match(/build_v(\d+)/);
    if (match) {
        dbName = `quiz_v${match[1]}.db`;
    }
    loadDatabase(dbName);
});
