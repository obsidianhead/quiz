...
// DB_BASE_URL will be replaced by build.bat for local/production
const dbBaseUrl = 'DB_BASE_URL';
...
function loadDatabase(dbName) {
    if (forceDBDownload) {
        console.log(`Forcing new download of ${dbName} from ${dbBaseUrl}...`);
        fetchDatabaseFromServer(dbName, dbBaseUrl);
    } else {
        fetchDatabaseFromIndexedDB(dbName).then(dbData => {
            if (dbData) {
                ...
                console.log(`${dbName} not found in IndexedDB, fetching from ${dbBaseUrl}...`);
                fetchDatabaseFromServer(dbName, dbBaseUrl);
            }
        }).catch(error => {
...
function fetchDatabaseFromServer(dbName, baseUrl) {
    ...
    fetch(dbPath)
        .then(response => {
            if (!response.ok) {
                throw new Error(`Failed to load ${dbName} from ${response.statusText}`);
            }
        ...
        .catch(error => {
            console.error(`Failed to load ${dbName} from ${response.statusText}:`, error);
        });
}
...
document.addEventListener('DOMContentLoaded', () => {
    // Extract version from dbBaseUrl if possible, else fallback to quiz.db
    let dbName = 'quiz.db';
    // Try to match _v<version> in any db file in the current directory
    const versionMatch = Array.from(document.getElementsByTagName('script')).map(s => s.src.match(/quiz_v(\d+)\.db/)).find(Boolean);
    if (versionMatch) {
        dbName = `quiz_v${versionMatch[1]}.db`;
    } else {
        // Try to match build_v<version> in dbBaseUrl
        const buildMatch = dbBaseUrl.match(/build_v(\d+)/);
        if (buildMatch) {
            dbName = `quiz_v${buildMatch[1]}.db`;
        }
    }
    loadDatabase(dbName);
});
